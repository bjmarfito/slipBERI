...
% ****** RUTH HACK - ACTUALLY I WANT ALL FREE EDGES
L1(L1==c) = c+r2;        % so what was '-4' I want to be '-3' ... or equivalent for non square fault patches.
% ****** end RUTH HACK 
L=L1;
...
L1=zeros(n,n*m);
L1(1:n,1:n)=eye(n)*r2;
L1(2:n,n+1:2*n-1)=eye(n-1);
L1(1:n,n+1:2*n)=L1(1:n,n+1:2*n)+eye(n)*c;
%if free_edge==2  % bottom row is free, so set bottom row center value to '-3' or equivalent
    L1(end,2*n)=c+r2;
%end
%if free_edge==1  % top row is free, so change top row center value to '-3' or equivalent
    L1(1,n+1)=c+r2;
%end
L1(1:n-1,n+2:2*n)=L1(1:n-1,n+2:2*n)+eye(n-1);
L1(1:n,2*n+1:3*n)=eye(n)*r2;
L=[L;L1];

for i=1:m-3
   L=[L;circshift(L1,[0,i*n])];
end 
...
% ****** RUTH HACK - ACTUALLY I WANT ALL FREE EDGES
L2(L2==c) = c+r2;        % so what was '-4' I want to be '-3' ... or equivalent for non square fault patches.
% ****** end RUTH HACK 
L=[L;L2];
...
LC(4,end-n)=r2;
% ****** RUTH HACK - ACTUALLY I WANT ALL FREE EDGES
LC(LC==c) = c+2*r2;        % so what was '-4' I want to be '-2' ... or equivalent for non square fault patches.
LC(LC==c+r2) = c+2*r2;        % so what was '-3' I want to be '-2' ... or equivalent for non square fault patches.
% ****** end RUTH HACK 
